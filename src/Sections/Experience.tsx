...
const ExperienceItem = ({ item, isSelected }: { item: typeof EXPERIENCES[0], isSelected: boolean }) => {
  ...
  return (
    <div
      ...
      className={`
        relative group transition-all duration-300 rounded-2xl shadow-xl shadow-gray-700/10 backdrop-blur-xl px-7 py-6 
        flex flex-col md:flex-row md:items-start gap-5 md:gap-8 border-2 overflow-hidden cursor-pointer
        ${isSelected ? 'scale-[1.02] border-blue-300/70 bg-white/70 dark:bg-neutral-900/60' : 'border-white/10 bg-white/40 dark:bg-neutral-900/40'}
      `}
      style={{
